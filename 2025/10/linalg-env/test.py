...
def solve_buttons(target, buttons):
    ...
    if opt.check() != sat:                                          # Optimize.check() (or model.check()) returns whether or not the set of constraints is satisfable.
        print("No solution")
        return None
    mod = opt.model()                                               # If the current Optimizer/Solver is satisifable, Optimize.model() returns the current inputs which satisfies the model's constraints.
...
# Additional Notes:
# Z3 also can create scopes using solver.push() and solver.pop(). This let's us add sets of constraints, check if the model is still satisfiable and get the solution, then roll the constraints back and test another set.
# Z3 can also work with Reals and Bools